               ...
               ResponseCollector
{
     ...
     * <tt>GoogleRelayedCandidateDatagramSocket</tt> handles STUN indications sent
    ...
    public void handleMessageEvent(StunMessageEvent e)
    {
        ...
        /* as REMOTE-ADDRESS and XOR-PEER-ADDRESS has the same attribute type
         * we cast it to XorPeerAddressAttribute but we do not apply XOR to get
         * the address
         */
        XorPeerAddressAttribute peerAddressAttribute
            = (XorPeerAddressAttribute)
                message.getAttribute(Attribute.REMOTE_ADDRESS);
        ...
        TransportAddress peerAddress
            = peerAddressAttribute.getAddress();
        if (peerAddress == null)
            return;
        ...
        synchronized (packetsToSend)
        {
            while (!closed)
            {
                        ...
                        packetToSendIndex++)
                {
                    ...
                    Request sendRequest
                        = MessageFactory.createSendRequest(username,
                            peerAddress,
                            data);
