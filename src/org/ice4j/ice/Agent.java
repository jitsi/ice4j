...
public class Agent
{
        ...
        if (knownPair != null)
        {
            ...
            if (knownPair.getState() == CandidatePairState.SUCCEEDED )
            {
                ...
                if (!isControlling() && triggerPair.useCandidateReceived())
                {
                    ...
                    //valid pair to true.
                    if( triggerPair.getState() == CandidatePairState.SUCCEEDED )
                        nominatePair( triggerPair );
                }
    ...
    /**
     * Looks for the specified {@link CandidatePair} with in the valid lists
     * of the media streams and sets its nominated flag to <tt>true</tt>.  This
     * may end ICE processing for this media stream as per Section 8 in RFC
     * 5245. The method has no effect if <tt>pairToNominate</tt> is not on any
     * valid list.
     *
     * @param pairToNominate the pair that we'd like to nominate in case it's on
     * a valid list somewhere.
     */
    public void nominatePair(CandidatePair pairToNominate)
    {
//implement todo !!!
    }
